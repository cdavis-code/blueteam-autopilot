...
**Narration:**
> "Installation is straightforward. Clone the repo, install Python dependencies, add your Qwen Cloud API key, and run the agent. That's it — all 7 agent skills with 15 demo fixtures are bundled locally. Alternatively, install as skills in your favorite AI IDE with a single npx command. The agent also connects to external MCP servers at startup, dynamically discovering tools from CISO Assistant and Alibaba Cloud without code changes."
...
**Narration:**
> "Alibaba Blueteam ships with its own standalone agent — a full terminal UI built on Qwen Cloud and ConnectOnion. Or, install the seven agent skills into your preferred AI IDE harness. Either way, you get the same tools, same scripts, and same demo fixtures."
