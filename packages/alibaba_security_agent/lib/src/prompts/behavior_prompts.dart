/// Focused sub-prompts for each agent behavior.
///
/// These can be composed into the full system prompt or used independently
/// for targeted agent invocations (e.g., a "triage-only" run).
///
/// Knowledge documents (compliance controls, runbooks, policies) are fetched
/// on-demand via the MCP `get_knowledge_document` tool **only when the user's
/// question or task requires detailed context** — see Knowledge Fetching
/// Policy in the system prompt.
class BehaviorPrompts {
  BehaviorPrompts._();

  /// Prompt fragment for incident discovery behavior.
  static String incidentDiscovery() => '''
# Incident Discovery
...
''';
  ...
  /// Prompt fragment for incident deep-dive behavior.
  static String incidentDeepDive() => '''
# Incident Deep-Dive
...
''';
  ...
  /// Prompt fragment for recommendation synthesis behavior.
  static String recommendationSynthesis() => '''
...
''';
  ...
  /// Prompt fragment for action proposal behavior.
  static String actionProposal() => '''
# Action Proposal
...
''';
  ...
  /// Prompt fragment for reporting behavior.
