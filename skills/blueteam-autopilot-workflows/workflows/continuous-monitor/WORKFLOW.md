...
1. Retrieve last check timestamp (harness-native — read from `data/monitor-state.json`
   or similar local file; if missing, this is the first run):
   - If null (first run), use the default time range (lastHour).
   - If set, use a time range that covers events since that timestamp.

2. Establish region and edition:
   ```bash
   python skills/blueteam-autopilot-ops/scripts/get_account_context.py
   ```

3. List security events with appropriate time range:
   ```bash
...
5. Cross-reference affected assets:
   ```bash
...
### Tool Invocations (per CRITICAL/HIGH event)
...
2. Get underlying alert grouping:
   ```bash
   python skills/blueteam-autopilot-ops/scripts/list_alerts.py <event_id>
   ```

3. Check institutional memory (harness-native — search saved incident records
...
4. Optionally check trusted networks:
   ```bash
...
- Full analysis using all 4 tool invocations above.
...
2. For each CRITICAL and HIGH event (harness-native):
   - Save a concise description to institutional memory including:
     attack type, severity, source IP, affected asset, novelty classification.

3. Update monitor state (harness-native — write to `data/monitor-state.json`):
   - Pass `escalations=critical_count + high_count`
   - Advance the `last_check_timestamp` for the next tick.

### Escalation Output

Produce a concise escalation summary for display:
...
The output should be concise enough for console display.
Use clear severity markers and keep descriptions to one line per event.
---
name: continuous-monitor
description: Autonomous SOC monitoring — scan for new events, triage severity, escalate high-severity findings
version: 1.0
