...
2. Discover cloud assets dynamically:
   ```bash
...
**IMPORTANT:** If `list_events.py` returns 0 events (empty SuspEvents array), this is
...
Produce a structured JSON inventory of all discovered events, affected assets,
...
1. Extract attack chain, source IPs, CVEs:
   ```bash
   python skills/blueteam-autopilot-ops/scripts/get_event_detail.py <event_id>
   ```

2. Get underlying alerts grouped by source:
   ```bash
...
3. For vulnerability-driven incidents:
   ```bash
...
Do NOT call `get_knowledge.py` for every security event. Call it ONLY when:
...
- Default to dry-run simulation unless user explicitly opts in
...
1. Cross-reference IPs against trusted networks FIRST:
   ```bash
   python skills/blueteam-autopilot-ops/scripts/get_knowledge.py trusted_networks
   ```

2. Preview the block action (dry-run):
   ```bash
   python skills/blueteam-autopilot-ops/scripts/block_waf_ips.py <ip1,ip2,...>
   ```

3. Present the dry-run result to the user for approval.

4. **Only after explicit user approval**, execute:
   ```bash
...
1. The system will prompt for human approval before executing:
   ```bash
   python skills/blueteam-autopilot-ops/scripts/execute_response_policy.py <policy_id> <event_id> --real
   ```

2. Log the action taken and its result.
...
Aggregate all investigation data from prior phases into a single structured report.
Use the event detail, alerts, assets, vulnerabilities, WAF data, and compliance
controls gathered throughout the workflow.

For storing findings in institutional memory:
- Save the report as a JSON file for future similarity search
- Include attack type, affected assets, severity, and key findings
