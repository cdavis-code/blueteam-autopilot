...
### Step 1: Create a Project Directory and Install Skills
...
This installs all 6 skills and their fixtures into your project. No repository cloning needed.
...
```bash
echo 'SECURITY_CENTER_MODE=demo' > .env  # Start with demo mode
```

For real mode (production use), add credentials:
...
> **Note for real mode:** You can also export these variables directly in your shell instead of using a `.env` file. The scripts check `$PWD/.env` first, then fall back to repo-relative paths.

### Step 3: Auto-Generate Configuration (Recommended)
